...
class CommentModel extends MainModel{
	...
	public function getComments($post_id){
		$sql = 'SELECT user_name, date_creation, content, state, id FROM comment  WHERE post_id=' . $post_id . " AND state = 2 OR state = 3  ORDER BY id DESC";
	...
	public function getCommentsAdmin($post_id){
		$sql = 'SELECT user_name, date_creation, content, state, id FROM comment  WHERE post_id=' . $post_id . " AND state = 0 OR state = 1 ORDER BY id DESC";
		$data = $this->db->query($sql);
		return $data->fetchAll();
	}
